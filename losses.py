...
class TripletForgetLoss(torch.nn.Module):
    """Triplet loss module."""

    def __init__(self, margin=1.0, p=2):
        """Initilize triplet loss module."""
        super(TripletForgetLoss, self).__init__()
        self.margin = margin
        self.p = p
        self.loss_fn = nn.TripletMarginLoss(margin=margin, p=p, reduction='none')

    def forward(self, anchor, positive, negative, forget_sample):
        """Calculate forward propagation.
        Args:
            anchor (Tensor): Anchor embeddings (B, D).
            positive (Tensor): Positive embeddings (B, D).
            negative (Tensor): Negative embeddings (B, D).
            forget_sample (Tensor): Boolean tensor indicating if the sample is from forget speaker (B,).
        Returns:
            Tensor: Triplet loss value.
        """
        loss = self.loss_fn(anchor, positive, negative)
        loss = loss * forget_sample
        loss = torch.sum(loss) / (torch.sum(forget_sample) + 1e-8)
        return loss